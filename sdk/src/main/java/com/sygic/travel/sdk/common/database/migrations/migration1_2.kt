...
val migration1_2 = object : Migration(1, 2) {
	override fun migrate(database: SupportSQLiteDatabase) {
		...
		database.execSQL("ALTER TABLE `favorite` ADD COLUMN `state` INTEGER NOT NULL DEFAULT 0")
		database.execSQL("UPDATE `favorite` SET `state` = 1") // mark all favorites for sync to server
	}
}
